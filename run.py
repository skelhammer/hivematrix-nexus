...
if __name__ == "__main__":
    ...
    # Unless DEV_MODE is enabled for live CSS/HTML reloading
    if (use_gunicorn or port == 443) and not dev_mode:
        ...
        # Live reloading enabled for CSS/HTML/Python changes
        if port == 443:
            cert_dir = os.path.join(os.path.dirname(__file__), 'certs')
            cert_file = os.path.join(cert_dir, 'nexus.crt')
            key_file = os.path.join(cert_dir, 'nexus.key')
            if os.path.exists(cert_file) and os.path.exists(key_file):
                ...
                print(f" * Using Flask development server (DEV_MODE - live reloading enabled)")
                app.run(host=host, port=port, debug=True, ssl_context=(cert_file, key_file))
            else:
                print(f" * ERROR: SSL certificates not found for port 443")
                sys.exit(1)
        else:
            print(f" * Nexus starting on http://{host}:{port}")
            print(f" * Using Flask development server (live reloading enabled)")
